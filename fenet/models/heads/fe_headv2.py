...
from fenet.utils.lane import Lane
from fenet.models.losses.focal_loss import FocalLoss
from fenet.models.losses.accuracy import accuracy
from fenet.ops import nms

from fenet.models.utils.roi_gather import ROIGather, LinearModule
from fenet.models.utils.seg_decoder import SegDecoder
from fenet.models.utils.dynamic_assign import assign
from fenet.models.losses.ldi_loss import ldi_iou_loss
from ..registry import HEADS
import pdb

@HEADS.register_module
class FEHeadV2(nn.Module):
    def __init__(self,
                 num_points=72,  #  feature point 72
                 prior_feat_channels=64,  # inner channels 64
                 fc_hidden_dim=64,  
                 num_priors=192,  # anchor
                 num_fc=2,  
                 refine_layers=3, 
                 sample_points=36,  
                 cfg=None):
        super(FEHeadV2, self).__init__()
        self.cfg = cfg
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        feature = feature.reshape(batch_size * num_priors,
                                  ...
                                  1)
        #  return [b*num_prior,c,sample_point,1]
        return feature
    ...
    def generate_priors_from_embeddings(self):
        ...
        # this part is start_y start_x theta length  [192,2-5]
        priors[:, 6:] = (
        ...
        # this part is 72 coordinate (x,y) [192,6-78]
    ...
    def forward(self, x, **kwargs):
        ...
        # batch_features 1 [24,64,40,100] = [b,c,h,w]
        # batch_features 2 [24,64,20,50]
        # batch_features 3 [24,64,10,25]

        batch_features.reverse()

        # batch_features 1 [24,64,10,25] = [b,c,h,w]
        # batch_features 2 [24,64,20,50]
        # batch_features 3 [24,64,40,100]

        batch_size = batch_features[-1].shape[0]
        # reverse index find the last one
        # batch_features [24,64,40,100] = [b,c,h,w]
        ...
        #   priors_on_featmap [24,192,36] = [bs,num_priors,sample_point]
        #  priors [24,192,78]
        ...
        #
        # start from stage 0-2  batch_features get from [0]-[2]
        for stage in range(self.refine_layers):
            num_priors = priors_on_featmap.shape[1]
            # num_priors 192
            prior_xs = torch.flip(priors_on_featmap, dims=[2])
            # prior_xs [24,192,36]   dim2 reverse 36 sequence

            # batch_features[stage] [24,64,40,100] = [b,c,h,w]
            batch_prior_features = self.pool_prior_features(
                batch_features[stage], num_priors, prior_xs)
            # batch_prior_features [4608,64,36,1] = [batch*num_priors,channel,sample_point,1]
            prior_features_stages.append(batch_prior_features)

            # prior_features_stages [4608,64,36,stage] = [batch*num_priors,channel,sample_point,stage]
            # input different stage prior and feature
            ...
            #  predictions [24,192,78]  <-  +cls_logits [24,192,2]
    ...
    def loss(self,
        ...
        if self.cfg.haskey('Piou_loss_weight'):
            Piou_loss_weight = self.cfg.Piou_loss_weight
        if self.cfg.haskey('seg_loss_weight'):
            seg_loss_weight = self.cfg.seg_loss_weight
        if self.cfg.haskey('Dliou_loss_weight'):
            Dliou_loss_weight = self.cfg.Dliou_loss_weight
        if self.cfg.haskey('Driou_loss_weight'):
            Driou_loss_weight = self.cfg.Driou_loss_weight
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                line_iou,l_iou,r_iou = ldi_iou_loss(
                    reg_pred, reg_targets,
                    self.img_w, length=15)

                # accumulate the loss
                P_iou = P_iou + line_iou
                Dl_iou = Dl_iou + l_iou
                Dr_iou = Dr_iou + r_iou
        ...
        P_iou /= (len(targets) * self.refine_layers)
        Dl_iou /= (len(targets) * self.refine_layers)
        Dr_iou /= (len(targets) * self.refine_layers)

        loss = cls_loss * cls_loss_weight + reg_xytl_loss * xyt_loss_weight \
               + seg_loss * seg_loss_weight + P_iou * Piou_loss_weight + Dl_iou * Dliou_loss_weight + Dr_iou * Driou_loss_weight
        ...
        return_value = {
            ...
            'loss_stats': {
                ...
                'Piou_loss': P_iou * Piou_loss_weight,
                'Dleft_iou': Dl_iou * Dliou_loss_weight,
                'Dright_loss': Dr_iou * Driou_loss_weight
